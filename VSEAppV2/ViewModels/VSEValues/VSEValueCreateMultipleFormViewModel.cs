...
{
    public class VSEValueCreateMultipleFormViewModel: ObservableRecipient
    {
        public VSEValueCreateMultipleFormViewModel(IAppState appState)
        {
            this.AppState = appState;

            this.AppState.OverallAppModel.LoginControl.VSEControl.VSEValueCreate += VSEControl_VSEValueCreate;

            this.CreateMultipleValuesCommand = new AsyncRelayCommand(CreateMultipleValuesCommand_Execute, CreateMultipleValuesCommand_CanExecute);
        ...
        private void VSEControl_VSEValueCreate(object sender, FormResponse_EventHandlerArg e)
        {
            ...
            if (notification == "OK")
            {
                this.StatusMessage = "Values Created";
            }
            else
            {
        ...
        {
            Messenger.Register<VSEValueCreateMultipleFormViewModel, OpenModal_VSEValue, string>(this, "CreateMultiple", (r, m) => Receive(m));
        }
        ...
        public bool IsLoading
        {
            ...
            set
            {
                ...
                this.CreateMultipleValuesCommand.NotifyCanExecuteChanged();
            }
        }
        ...
        public void Unloaded()
        {
            ...
            this.IsActive = false;
            Messenger.Unregister<OpenModal_VSEValue, string>(this, "CreateMultiple");
        }

        public AsyncRelayCommand CreateMultipleValuesCommand { get; }
        private async Task CreateMultipleValuesCommand_Execute()
        {
            this.IsLoading = true;
            HttpContent createMultipleFormContent = this.GenerateHttpContent();
            await this.AppState.OverallAppModel.LoginControl.VSEControl.CreateVSEDataAndNotify(VSEType.Value, createMultipleFormContent);
        }
        private bool CreateMultipleValuesCommand_CanExecute()
        {
